(function () {
    ...
    const store = {
        get target() {
            return TARGETS.some(t => t.code === settings.target_currency) ? settings.target_currency : 'SAR';
        },
        set target(code) { save('target_currency', code); },
        get lang() { return settings.lang === 'en' ? 'en' : 'ar'; },
        set lang(code) { save('lang', code === 'en' ? 'en' : 'ar'); },
        get originalColor() { return !!settings.original_color; },
        set originalColor(value) { save('original_color', !!value); },
        get debug() { return !!settings.debug; },
        set debug(value) { save('debug', !!value); },
        get accepted() { return !!settings.terms_accepted; },
        set accepted(value) { save('terms_accepted', value ? 'ok' : ''); },
    };
    ...
    function requestRates(force) {
        return new Promise((resolve) => {
            let settled = false;
            const finish = (value) => { if (!settled) { settled = true; resolve(value); } };
            setTimeout(() => finish(null), 12000);
            try {
                chrome.runtime.sendMessage({ type: 'WDSTEAMFX_GET_RATES', force: !!force }, (response) => {
    ...
    async function getRates(force) {
        const response = await requestRates(force);
        if (response && response.rates) {
            log('rates from', response.source, response.rates);
            return {
                rates: Object.assign({}, DEFAULT_RATES, response.rates),
                source: response.source || null,
                at: response.at || 0,
            };
        }
        return { rates: Object.assign({}, DEFAULT_RATES), source: null, at: 0 };
    }
    ...
    try {
        chrome.storage.local.get(Object.keys(settings), (saved) => {
            ...
            TARGET = TARGETS.find(t => t.code === store.target) || TARGETS[0];
            boot();
        });
    } catch (e) {
        boot();
